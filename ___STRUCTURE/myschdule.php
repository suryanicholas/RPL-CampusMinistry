...
<head>
    ...
    <style>
        <?php include_once "___DESIGN/myschdule.css"; ?>
    </style>
</head>
<body>
    <div class="container">
            ...
            <div class="content-body">
                <div class="content-table">
                    <div class="table-head">
                        <div class="title">Nama Konselor</div>
                    ...
                    <div class="table-body">
                        <?php
                            $_letQUERY = $_DC->prepare("SELECT info.cmUsername, data.csDataID, data.csDate, data.csSesi, data.csMetode, data.csSTATUS FROM cmuserinfo info INNER JOIN csdata data ON info.cmUserID = data.csKonselor WHERE data.csUserID = $ownUSERID");
                            $_letQUERY->execute();
                        ...
                        ?>
                            <div class="table-row">
                                ...
                                <div class="table-column"><?php echo $row['csMetode'] ?></div>
                                <div class="table-column">
                                    <?php
                                        if($row["csSTATUS"] === "R"){
                                    ?>
                                            <a href="<?php echo $_URL ?>/layanan.php?jadwalku=<?php echo bin2hex(base64_encode($row["csDataID"])) ?>&action=clear">Hapus</a>
                                    <?php
                                        } else {
                                    ?>
                                            <a href="<?php echo $_URL ?>/layanan.php?jadwalku=<?php echo bin2hex(base64_encode($row["csDataID"])) ?>&action=cancel">Batalkan</a>
                                    <?php
                                        }
                                    ?>
                                </div>
